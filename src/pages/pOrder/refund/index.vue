<template>
  <div class="container">
    ...
    <div class="third box">
      ...
      <div class="uploading" @click="chooseImageTap"  v-for="pic in pathPics" >
        <img :src="pic" alt="">
        
      </div>
      <div class="upload" @click="chooseImageTap" v-if="pathPics.length <9">
    ...
    <div class="last box">
      <div class="items"><span>联系电话</span><span><input class="items-phone" type="text" v-model="rePhone" placeholder="请输入手机号" placeholder-style="color:#ccc;"></span></div>
    </div>
...
export default {
  data () {
    return {
      aloneflag: false, // 部分退款
      chooseImage: true, // 选择图片的占位
      refundTypeStr: '请选择', //  退款类型
      refundTypeValue: [
        { str: '整单', value: '10' },
        { str: '部分', value: '20' }],
      refundReasonValue: [ // 退款原因
        { str: '我不想买了', value: '28' },
        { str: '商家缺货', value: '1' },
        { str: '质量问题', value: '2' },
        { str: '包装问题', value: '3' },
        ...
        { str: '其他原因', value: '5' }],
      refundReasonStr: '请选择',
      refundTypeStatus: false, // 弹层状态
      refundReasonStatus: false, // 弹层状态
      // 退款参数
      orderId: '', // 订单Id
      goodsStr: '', // 商品编号
      pathPics: [], // 图片地址数组
      refundMeth: '', // 退款类型 退款  / 退款退货
      refundReason: '', // 退款原因
      refundScope: '', //
      refundType: '', // 退款类型   整单  /部分
      listGoods: '', // 商品列表
      money: '', // 退款金额
      rePhone: '', // 退款手机号
      isShow: false,
      isRefund: false, // 是否可以退款
      goodList: [] // 商品列表副本
    }
  },
  computed: {
  ...
  methods: {
    ...
    refundApplyRequest: async function () {
      this.userInfo = await getMyAccount()
      var userId = this.userInfo.result.result.member.mid
      this.rePhone = this.userInfo.result.result.member.mphone
      console.log(this.userInfo.result.result.member.mid)
    ...
    chooseImageTap: function () {
      ...
      wx.chooseImage({
        ...
        success (res) {
          ...
          that.pathPics.push(...tempFilePaths)
          console.log(tempFilePaths)
        }
      })
      console.log(this.pathPics.join(','))
    ...
    goodRefundApply: async function () {
      ...
      var pathPics = this.pathPics.toString() //= '/Uploads/ueditor/images/20191203/1575356506959002590.jpg'
      ...
      var applyPhone = this.rePhone
      var result = await goodRefundApply({ orderId, goodsStr, pathPics, refundMeth, refundReason, refundScope, refundType, applyPhone })
